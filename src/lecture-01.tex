...
\item Substitute the conditional expectation (\xref{1.4.8}),
...
\item The outer integrand carries the factor $f_X(x)$, so the set where $f_X(x)=0$
      ...
      conditional density (\xref{1.4.6}), $f_{Y\mid X}(y\mid x)=f(x,y)/f_X(x)$:
      \[ \E\{\E\{Y\mid X\}\}
...
\item The inner integral is the marginal density of $Y$ (\xref{1.4.5}, with the roles of $X$
