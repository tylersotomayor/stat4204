...
provided the sum or integral converges absolutely: the probability-weighted average of the
...
\begin{Obj}{Theorem}{1.4.3}{The law of the unconscious statistician}
...
\end{Obj}
\begin{Prf}{1.4.3}{Proof by Regrouping of the Unconscious-Statistician Formula (discrete case).}{}
...
\begin{Obj}{Definition}{1.4.4}{The joint distribution of a pair}
...
\begin{Obj}{Definition}{1.4.5}{Marginal distribution}
...
\begin{Obj}{Definition}{1.4.6}{Conditional distribution}
...
\begin{ObjL}{Remark}{1.4.7}{Conditional distributions are probability distributions!}
...
\begin{Obj}{Theorem}{1.4.9}{Iterated expectations}
Whenever the expectations exist,
\[ \E\{\E\{Y\mid X\}\} \;=\; \E\{Y\}. \]
\end{Obj}
\begin{Prf}{1.4.9.a}{Proof by Conditioning of the Iterated-Expectations Identity (discrete case).}{}
\item $\E\{Y\mid X\}$ is the function $x\mapsto\E\{Y\mid X=x\}$ evaluated at $X$, so by the law
      of the unconscious statistician (\xref{1.4.3}),
      \[ \E\{\E\{Y\mid X\}\} \;=\; \sum_x \E\{Y\mid X=x\}\,p_X(x) . \]
\item Substituting \xref{1.4.8} and \xref{1.4.6},
      \[ \sum_x \sum_y y\,p_{Y\mid X}(y\mid x)\,p_X(x) \;=\; \sum_x\sum_y y\,p(x,y) . \]
\item Exchanging the order of summation (legitimate by the assumed absolute convergence) and
      summing out $x$ gives
      \[ \sum_y y \sum_x p(x,y) \;=\; \sum_y y\,p_Y(y) \;=\; \E\{Y\} . \]
\end{Prf}
\begin{Prf}{1.4.9.b}{Proof by Conditioning of the Iterated-Expectations Identity (continuous case).}{}
\item As before, $\E\{Y\mid X\}$ is a function of $X$, so by the law of the unconscious
      statistician (\xref{1.4.3}), now in its integral form,
      \[ \E\{\E\{Y\mid X\}\} \;=\; \int \E\{Y\mid X=x\}\,f_X(x)\,dx . \]
\item Substituting \xref{1.4.8} and \xref{1.4.6}, the conditional density's denominator
      cancels against the marginal wherever $f_X(x)>0$---and the set where $f_X(x)=0$
      contributes nothing to the outer integral:
      \[ \int\!\left(\int y\,f_{Y\mid X}(y\mid x)\,dy\right) f_X(x)\,dx
         \;=\; \int\!\!\int y\,f(x,y)\,dy\,dx . \]
\item Exchanging the order of integration (legitimate, by Fubini's theorem, under the assumed
      absolute convergence) and integrating out $x$ gives
      \[ \int y \int f(x,y)\,dx\,dy \;=\; \int y\,f_Y(y)\,dy \;=\; \E\{Y\} . \]
\end{Prf}

% ============================================================
\sub{1.5}{Decision Theory: Actions, Loss, and Risk}

Decision theory begins from the statistical model of \xref{1.3.1}---a ``parameterized set of
distributions on the sample space''---and adds three objects. \HPS{1}
...
\begin{Obj}{Principle}{1.5.5}{The risk principle}
...
\begin{Obj}{Definition}{1.5.6}{Admissibility}
...
\begin{Fig}{1.5.7}{Three risk functions}{Risk curves for three decision rules, as sketched in
...
dominates the other, and the deck's legend marks both admissible (\xref{1.5.6}).}
...
\begin{ObjL}{Concept}{1.5.8}{Should we follow the normative prescription?}
...
\item Finally, the action $a$ minimizing the posterior expected loss
      ...
      \xref{1.4.9}), minimizing the latter for each $x$ minimizes the former, so $T$ is the
      Bayes rule.
\end{Prf}
