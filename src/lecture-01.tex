...
$p(x_1,\dots,x_n)=P(X_1=x_1,\dots,X_n=x_n)$; in the continuous case by a \emph{joint density}
$f\ge0$ with
...
\begin{Obj}{Theorem}{1.4.2}{The law of the unconscious statistician}
...
\end{Obj}
\begin{Prf}{1.4.2}{Proof by Regrouping of the Unconscious-Statistician Formula (discrete case).}{[A11]}
...
\begin{Obj}{Definition}{1.4.3}{Joint, marginal, and conditional distributions}
For a pair $(X,Y)$ with joint pmf $p(x,y)$: the \emph{marginal} pmf of $X$ is
$p_X(x)=\sum_y p(x,y)$, and for any $x$ with $p_X(x)>0$ the \emph{conditional} pmf of $Y$ given
$X=x$ is
\[ p_{Y\mid X}(y\mid x) \;=\; \frac{p(x,y)}{p_X(x)} . \]
Densities work the same way, with integrals in place of sums.
\end{Obj}

\begin{ObjL}{Remark}{1.4.4}{Conditional distributions are probability distributions!}
...
\begin{Obj}{Definition}{1.4.5}{Conditional expectation}
...
\begin{Obj}{Theorem}{1.4.6}{Iterated expectations}
Whenever the expectations exist,
\[ \E\{\E\{Y\mid X\}\} \;=\; \E\{Y\}. \]
\end{Obj}
\begin{Prf}{1.4.6}{Proof by Conditioning of the Iterated-Expectations Identity (discrete case).}{[A1$+$A11]}
\item $\E\{Y\mid X\}$ is the function $x\mapsto\E\{Y\mid X=x\}$ evaluated at $X$, so by the law
      of the unconscious statistician (\xref{1.4.2}),
      \[ \E\{\E\{Y\mid X\}\} \;=\; \sum_x \E\{Y\mid X=x\}\,p_X(x) . \]
\item Substituting \xref{1.4.5} and \xref{1.4.3},
...
\end{ObjL}
\begin{Prf}{1.A.1}{Proof of the Bias--Variance Decomposition by Adding and Subtracting the Mean.}{[A4$+$A2]}
...
\end{ObjL}
\begin{Prf}{1.A.2}{Proof of the Rate-Difference Risk via Unbiasedness and Independence.}{[A2$+$A6]\quad uses \xref{1.A.1}.}
...
\end{ObjL}
\begin{Prf}{1.A.3}{Proof of the Shaded Estimator's Risk by Linear-Combination Rules.}{[A2$+$A4]\quad uses \xref{1.A.1}, \xref{1.A.2}.}
...
\end{enumerate}
\end{ObjL}
\begin{Prf}{1.A.4}{Proof of the Bayes Risks by Integration, and of the Bayes Rule by Identifying the Posterior.}{[A1$+$A11]\quad uses \xref{1.A.2}, \xref{1.A.3}, \xref{1.8.5}.}
...
\item Finally, the action $a$ minimizing the posterior expected loss
      ...
      \xref{1.4.6}), minimizing the latter for each $x$ minimizes the former, so $T$ is the
      Bayes rule.
\end{Prf}
